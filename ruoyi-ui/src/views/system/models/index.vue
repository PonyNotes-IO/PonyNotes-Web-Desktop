<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="modelsList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="API调用地址" align="center" prop="apiUrl" />
      <el-table-column label="状态" align="center" prop="status">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_app_status" :value="scope.row.status"/>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="API调用地址" prop="apiUrl">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        apiUrl: [
          { required: true, message: "API调用地址不能为空", trigger: "blur" }
        ]
      }
    }
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除AI模型管理编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/models/export', {
        ...this.queryParams

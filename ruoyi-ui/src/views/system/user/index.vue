...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      // 根据员工账号进行精准搜索，根据邮箱和电话进行模糊查询
      const params = {
        ...this.queryParams,
        userName: this.queryParams.userName ? this.queryParams.userName : undefined,
        email: this.queryParams.email ? this.queryParams.email : undefined,
        phonenumber: this.queryParams.phonenumber ? this.queryParams.phonenumber : undefined
      }
    ...
    /** 获取部门列表 */
    getDeptList() {
      import("@/api/system/dept").then(({ listDept }) => {

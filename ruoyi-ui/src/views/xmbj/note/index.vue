...
<script>
import { listNote, getNote, delNote, addNote, updateNote } from "@/api/xmbj/note"
...
export default {
  ...
  created() {
    this.workspaceId = this.$route.query.workspaceId || null
    if (this.workspaceId) {
      this.queryParams.workspaceId = this.workspaceId
    }
    this.getList()
  },
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除快速笔记编号为"' + ids + '"的数据项？').then(function() {
        return delNote(ids)
      }).then(() => {
        this.getList()
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('xmbj/note/export', {
        ...this.queryParams

    ...
    <div class="xm-index-nav-bar" :class="{ 'is-sticky': isSticky }">
      <div class="content-bar">
        <div class="box_23 flex-row group_1">
          ...
          <!-- 登录状态：显示用户信息图标（点击退出） -->
          <div class="user-info-wrapper flex-col" v-if="userInfo">
...
<script>
import {  TokenKey, getToken,  UserInfoKey,  } from '@/utils/auth';
export default {
  data() {
    return {
  ...
  computed: {
    ...
    showLoginButton() {
      ...
      if (localStorage.getItem(TokenKey) == null || localStorage.getItem(TokenKey) == '') {
        return true;
      }else {
        return false;
      }

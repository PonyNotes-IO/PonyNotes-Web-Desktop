...
// import { sendCaptcha } from '../../utils/captchaUtil.js';
import { getuserinfo,sendCaptcha } from '../../api/ponynote_login.js'; 
export default {  
  name: 'Login',
  ...
  methods: {
    ...
    async handleLoginRegister(){
      ...
      try {
        ...
        // const response = await this.$axios.post('/api/getuserinfo', {
        ...
        // 2. 根据后端返回的账号状态分支处理
        console.log(this.isNewAccount , this.isFirstEmailLogin ,this.isPhoneNumber)
        ...
        if (!nextPath) {
          this.$toast.fail('无法确定跳转路径，请重试')
          return
        }
        if(nextPath === '/login/login_verify_code'){//新人登录进入发送验证码
        ...
        this.phone  = (this.isPhoneNumber ? this.inputValue : null)
        this.$router.push({
            path:nextPath,
            query: {
            accountType: accountType,  // 账号类型

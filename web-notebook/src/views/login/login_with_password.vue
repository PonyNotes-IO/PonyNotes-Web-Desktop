...
export default {
  ...
  methods: {
    ...
    async handleLogin() {
      ...
      try {
        // this.accountType === 'phone' ? this.loginPhonePassword() : this.loginEmailPassword();
        // 调用登录接口
    ...
    // 跳转到验证码重置密码页面
    goToResetPassword() {
      let sendCaptchaResult = this.sendCaptchaAction(this.accountType,this.account)
      console.log(sendCaptchaResult)
      this.$router.push({
        path: '/login/login_verify_code',
    ...
    async  sendCaptchaAction(accountType,account){ 
